    ...
    lobbyForm.on("submit", (e) => {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                $("#updated-account").text("Aplicado!")
                $("#updated-account").show()
            },
            error: (data) => {
                clearInterval(timeout)
                setTimeout(() => $("#updated-account").hide(), 5000)
                $("#updated-account").text("No se pueden aplicar los cambios.")
                $("#updated-account").show()
            }
        })
    })
            ...
            error: (data) => {
                switch (JSON.parse(data.responseText).errorCode) {
                    case "dev.aurorafn.id.gift.invalid_fields":
                        $("#gift-notice").text("Error: Porfavor llena todos los casilleros.")
                        $("#gift-notice").show()
                        break;
                    case "dev.aurorafn.id.gift.giftbox_not_found":
                        $("#gift-notice").text("Error: No se encontró la caja de regalos.")
                        $("#gift-notice").show()
                        break;
                    case "dev.aurorafn.id.gift.cosmetic_not_found":
                        $("#gift-notice").text("Error: No se encuentra el cosmeticos.")
                        $("#gift-notice").show()
                        break;
                    case "dev.aurorafn.id.gift.account_not_found":
                        $("#gift-notice").text("Error: Usuario no encontrado.")
                        $("#gift-notice").show()
                        break;
                    case "dev.aurorafn.id.gift.friendship_not_found":
                        $("#gift-notice").text("Error: No hay amigos en esta cuenta.")
                        $("#gift-notice").show()
                        break;
                    case "dev.aurorafn.id.gift.account_too_many_gifts":
                        $("#gift-notice").text("Error: HAY DEMACIADOS REGALOS A ESTA CUENTA!.")
                        $("#gift-notice").show()
                        break;
                    case "dev.aurorafn.id.gift.account_gifting_disabled":
                        $("#gift-notice").text("Error: Tienes los regalos deshabilitados.")
                        $("#gift-notice").show()
                        break;
                    default:
                        $("#gift-notice").text("Error: No se pueden enviar los regalos.")
                        $("#gift-notice").show()
                        break;
                }
            },
